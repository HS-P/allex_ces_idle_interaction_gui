...
class GazeControllerNode(Node):
    ...
    def __init__(self):
        ...
        self.tau_waist = 0.29  # TRACKING용 지연 (현재의 50%: 0.167 / 0.5 = 0.334초)
        ...
        self.tau_neck_cmd = 0.098  # TRACKING 기준: 0.14 * 0.7 = 0.098초 (30% 감소, 더 빠른 반응)
        ...
        try:
            match state:
                ...
                case TrackingState.LOST:
                    ...
                    if target_info.point is not None and target_info.track_id is not None:
                        ...
                        if target_info.track_id == self.lost_last_track_id:
                            ...
                            self.lost_same_target_time = None
                            self.get_logger().debug(
                                f"[LOST] 다른 타겟 무시: track_id={target_info.track_id} (이전: {self.lost_last_track_id})"
                            )
                    else:
                        # 타겟이 없으면 대기 시간 초기화
                        self.lost_same_target_time = None
                ...
                # IDLE 상태: 영자세(0도)로 50초 동안 천천히 선형 이동 (10% 속도)
                    ...
                    max_speed_per_sec = math.radians(1.0)  # 초당 1도
                    dt_idle = 1.0 / 60.0  # 60Hz 제어 루프
                    max_delta_per_frame = max_speed_per_sec * dt_idle  # 프레임당 최대 변화량
                    ...
                    # 허리도 동일하게 피드백 제어
                    if abs(error_waist) > max_delta_per_frame:
                        new_waist_yaw = current_waist_yaw + math.copysign(max_delta_per_frame, error_waist)
                    else:
