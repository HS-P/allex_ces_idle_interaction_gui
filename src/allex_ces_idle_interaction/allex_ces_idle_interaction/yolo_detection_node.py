...
class YOLODetectionNode(Node):
    ...
    def __init__(self):
        ...
        # BotSort 설정 파일 경로 초기화 (한 번만 설정하여 재사용)
        import os
        ...
        if not os.path.exists(self.tracker_config_path):
            self.get_logger().error(f"❌ BOTSORT 설정 파일을 찾을 수 없음: {self.tracker_config_path}")
            raise FileNotFoundError(f"BOTSORT 설정 파일이 필요합니다: {self.tracker_config_path}")
        self.get_logger().info(f"BotSort 설정 파일: {self.tracker_config_path}")
    ...
    def _init_yolo_model(self):
        ...
        _ = self.yolo_model.track(
            ...
            verbose=False, 
            imgsz=640,
            persist=True,  # tracker 상태 유지
            tracker=self.tracker_config_path  # BotSort 설정 파일 사용 (중요!)
        )
    ...
    def image_callback(self, msg: CompressedImage) -> None:
        ...
        # BOTSORT 설정 파일은 __init__에서 이미 설정됨 (재사용)
        results = self.yolo_model.track(
            frame,
            conf=self.conf_threshold,
            classes=[PERSON_CLASS_ID],   # 사람만
            persist=True,                # 내부 tracker 상태 유지
            tracker=self.tracker_config_path,  # BOTSORT 설정 파일 사용 (재사용)
            verbose=False,
            imgsz=640,
        )
